/// https://github.com/syscoin/eth-proof/blob/master/verifyProof.js

import { math } from "near-runtime-ts";
import { rlp } from 'rlp';

const keccak256 = math.keccak256;
...
VerifyProof.header = (header, blockHash) => {
    try {
        return Buffer.from(keccak256(rlp.encode(header))).equals(blockHash);
    } catch (e) {
        console.log(e);
    }
    return false;
...
VerifyProof.code = (address, code, parentNodes, header, blockHash) => {
    try {
        ...
        if (Buffer.from(keccak256(code)).equals(account[3])) {
...
VerifyProof.storageAtIndex = (storageIndex, storageValue, storageParentNodes, address, accountParentNodes, header, blockHash) => {
    try {
        var storagePath = Buffer.from(keccak256(Buffer.from(leftPad(storageIndex.toString('hex')), 'hex')));
...
VerifyProof.storageMapping = (storageIndex, mappings, storageValue, storageParentNodes, address, accountParentNodes, header, blockHash) => {
    try {
        ...
        pathBuilder = Buffer.from(keccak256(pathBuilder));

        var storagePath = Buffer.from(keccak256(pathBuilder));
        return VerifyProof.storage(storagePath, storageValue, storageParentNodes, address, accountParentNodes, header, blockHash);
    } catch (e) {
        console.log(e);
    }
...
VerifyProof.account = (address, account, parentNodes, header, blockHash) => {
    var sha3OfAddress = Buffer.from(keccak256(address));
...
VerifyProof.trieValue = (path, value, parentNodes, root) => {
    try {
        ...
        for (var i = 0; i < len; i++) {
            ...
            if (!nodeKey.equals(Buffer.from(keccak256(rlp.encode(currentNode))))) {
                return false;
            }
            ...
            switch (currentNode.length) {
                case 17://branch node
                    ...
                    nodeKey = currentNode[parseInt(path[pathPtr], 16)]; //must == keccak256(rlp.encode(currentNode[path[pathptr]]))
                    pathPtr += 1;

                    break;
                case 2:
                    var nibblesint = nibblesToTraverse(currentNode[0].toString('hex'), path, pathPtr);
                    pathPtr += nibblesint
